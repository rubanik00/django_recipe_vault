...
class PublicUserApiTests(TestCase):
    ...
    def test_user_password_too_short_error(self):
        """Test creating user with password less than 8 chars fails"""
        payload = {
            'email': 'test@example.com',
            'password': 'test',
            'name': 'Test Name',
    ...
    def test_create_token_invalid_credentials(self):
        ...
        payload = {
            'email': user_payload['email'],
            'password': "testFake123",
        }
        res = self.client.post(TOKEN_URL, payload)
        self.assertNotIn('token', res.data)
        self.assertEqual(res.status_code, status.HTTP_400_BAD_REQUEST)

    def test_create_token_no_user_error(self):
...
class PrivateUserApiTests(TestCase):
    ...
    def test_update_user_profile(self):
        ...
        payload = {'name': "Test New Name", 'password': "newTestPass123"}
